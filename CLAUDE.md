...
- **`launcher::build_ssh_command` takes an `update_mode: bool`.** When true, the SSH command forces a TTY (`-t`), appends `'<OPENCLAW_UPDATE_CMD>'` as a single shell-quoted argument, and the connection exits when the remote command returns. Both call sites (`launch_bookmark` Tauri command and `handle_tray_menu_event`) read `update_mode` from `AppState.settings`; if you add a third call site, do the same. `OPENCLAW_UPDATE_CMD` is a `pub const` so it's also surfaced through `SettingsView.openclaw_update_cmd` to the frontend (the prefs modal renders it verbatim).
